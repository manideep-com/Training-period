  ...
  try {
    ...
    return referenceTokens.reduce((current, referenceToken, referenceTokenPosition) => {
      if (realm.isArray(current)) {
        ...
        if (strictArrays && !(0, _arrayIndex.default)(referenceToken)) {
        ...
        if (!realm.has(current, referenceToken) && strictArrays) {
          throw new _JSONPointerIndexError.default(`Invalid array index "${referenceToken}" at position ${referenceTokenPosition} in "${jsonPointer}": out of bounds`, {
            jsonPointer,
            referenceTokens,
            referenceToken,
            referenceTokenPosition,
